import jstracker from './js-tracker'; // 跟踪js代码
import rule from './rule'; // 校验上报数据是否合法
...
const LOG_TYPE_PRODUCT = 'product'; // 产品指标 pv uv 用户停留指标
...
const DEFAULT_CONFIG = {
  ...
  // 因此需要业务方传入一个处理函数, 根据当前url解析出真实的页面类型, 以便系统对错误来源进行分类
...
const validLog = (type = '', code, detail = {}, extra = {}) => {
  ...
  if (ruleItem) {
    // 消费字段必填
    const requireFields = [...ruleItem.df]; //defail field
...
log.set = (customerConfig = {}, isOverwrite = false) => {
  ...
  const checkErrrorNeedReportFunc = _.get(commonConfig, [
    'record',
    'js_error_report_config',
    'checkErrrorNeedReport',
  ]);
  if (_.isFunction(checkErrrorNeedReportFunc) === false) {
    debugLogger(
      '警告: config.record.js_error_report_config.checkErrrorNeedReport 不是可执行函数, 将导致错误打点数据异常'
    );
  }
  ...
  report: function (errorLogList = []) {
    ...
    for (let errorLog of errorLogList) {
      ...
      try {
        ...
        debugLogger(
          `config.record.js_error_report_config.checkErrrorNeedReport执行时发生异常, 请注意, 页面报错信息为=>`,
          { e, desc, stack }
        );
        isNeedReport = true;
      }
      if (isNeedReport === false) {
        debugLogger(
          `config.record.js_error_report_config.checkErrrorNeedReport返回值为false, 跳过此类错误, 页面报错信息为=>`,
          { desc, stack }
        );
        continue;
      }
